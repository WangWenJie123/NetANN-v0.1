...
# sift1M_DATASET_PATH = '/home/wwj/Vector_DB_Acceleration/nvme_vector_datasets/sift1M/'
...
# gist_DATASET_PATH = '/home/wwj/Vector_DB_Acceleration/nvme_vector_datasets/gist/'
...
sift200M_DATASET_PATH = '/home/wwj/nvme_ssd_test_dir/sift1B/'
# sift200M_DATASET_PATH = '/home/wwj/Vector_DB_Acceleration/nvme_vector_datasets/sift1B/'
...
# NPROB_LIST = [4, 8, 16, 32, 64, 128, 256, 512]
...
def bvecs_read_500M_base(fname, c_contiguous=True):
   fv = np.fromfile(fname, dtype=np.byte)
   if fv.size == 0:
       return np.zeros(0, 0)
   dim = fv.view(np.int32)[0]  # 更改数据类型为int32,且查看该数组的第一个维度
...
def create_collection(name, id_field, vector_field, _DIM):
    ...
    collection = Collection(name=name, data=None, schema=schema, properties={"collection.ttl.seconds": 80000000000})
...
def insert(collection, xb_num, xb_dim, xb_vector_data, xq_num, xq_dim, xq_vector_data):
    batch_size = 50000
    print("xb data size is ", xb_num)
    print("xq data size is ", xq_num)
...
def set_properties(collection):
    collection.set_properties(properties={"collection.ttl.seconds": 80000000000})
...
def main():
    ...
    if(sys.argv[1] == 'sift1M'):
        xb, xq, gt = load_sift1M(sift_base=sift1M_DATASET_PATH+sift1M_base_file, sift_query=sift1M_DATASET_PATH+sift1M_query_file, sift_learn=sift1M_DATASET_PATH+sift1M_learn_file, sift_gt=sift1M_DATASET_PATH+sift1M_gt_file)
        
    if(sys.argv[1] == 'gist'):
        xb, xq, gt = load_sift1M(sift_base=gist_DATASET_PATH+gist_base_file, sift_query=gist_DATASET_PATH+gist_query_file, sift_learn=gist_DATASET_PATH+gist_learn_file, sift_gt=gist_DATASET_PATH+gist_gt_file)
        
    if(sys.argv[1] == 'sift200M'):
        xb = bvecs_read_base(sift200M_DATASET_PATH + sift200M_base_file)
        xq = bvecs_read_query(sift200M_DATASET_PATH + sift200M_query_file)
        gt = ivecs_read(sift200M_DATASET_PATH + sift200M_gt_file)
    
    if(sys.argv[1] == 'sift500M'):
        xb = bvecs_read_500M_base(sift200M_DATASET_PATH + sift200M_base_file)
        xq = bvecs_read_query(sift200M_DATASET_PATH + sift200M_query_file)
        gt = ivecs_read(sift200M_DATASET_PATH + sift200M_gt_file)
    ...
    for _NLIST in NLIST_LIST:
        ...
        if has_collection("sift500M"):
            drop_collection("sift500M")
