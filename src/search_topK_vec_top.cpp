...
#define MAX_CENTROIDS_NUM 61440
...
// void search_topK_vec_top(hls::stream<pkt>& local_in_xq_vector, int* mem_CentroidsVector, int* oputCentroids_id, unsigned int numCentroids, unsigned int dim, unsigned int nprobe)
// void search_topK_vec_top(int* local_in_xq_vector, int* mem_CentroidsVector, int* oputCentroids_id, unsigned int numCentroids, unsigned int dim, unsigned int nprobe, int* out_topk_dis)
void search_topK_vec_top(TYPE* local_in_xq_vector, TYPE* mem_CentroidsVector, int* oputCentroids_id, unsigned int numCentroids, unsigned int dim, unsigned int nprobe, int* out_topk_dis)
{
// #pragma HLS INTERFACE m_axi port = local_in_xq_vector offset = slave bundle = gmem
// #pragma HLS INTERFACE m_axi port = mem_CentroidsVector offset = slave bundle = gmem
#pragma HLS INTERFACE m_axi port = local_in_xq_vector offset = slave bundle = gmem max_read_burst_length = 64 num_read_outstanding = 16
...
#pragma HLS INTERFACE m_axi port = out_topk_dis offset = slave bundle = gmem
#pragma HLS INTERFACE s_axilite port = local_in_xq_vector
...
#pragma HLS unroll factor=2
        ...
        // pkt xq_vec = local_in_xq_vector.read();
        // local_xqVector[i] = xq_vec.data;
        // local_xqVector[i] = local_in_xq_vector[i];
        // local_xqVector << local_in_xq_vector[i];
...
#pragma HLS unroll factor=2
        ...
        out_topk_dis[i] = outL2dis[local_oputCentroids_id[i]];
        // oputCentroids_id[i] = local_oputCentroids_id.read();
        // out_topk_dis[i] = outL2dis_out[oputCentroids_id[i]];
    }
}
